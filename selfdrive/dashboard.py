...
def dashboard_thread(rate=100):
  ...
  can = messaging.sub_sock(context, service_list['can'].port, addr=ipaddress, conflate=True, poller=poller)
  ...
  while 1:
    ...
    for socket, event in poller.poll(0):
      if socket is live100:
        ...
        if sample_str != "":
            sample_str += ","
        sample_str += ("angleSteersDes=%f,vEgo=%f,steerOverride=%f,vPid=%f,upSteer=%f,uiSteer=%f,ufSteer=%f,cumLagMs=%f,vCruise=%f" %
                    (_live100.live100.angleSteersDes, _live100.live100.vEgo, _live100.live100.steerOverride, _live100.live100.vPid,
    ...
    if frame_count >= 20:
        ...
        try:
            return = influx.request("write",'POST', {'db':DBNAME}, influxLineString.encode('utf-8'), 204, headers)
            print(return)
